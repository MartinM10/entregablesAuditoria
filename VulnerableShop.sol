...
contract VulnerableShop is Ownable {
    ...
    */
    ...
    function newSale(string calldata title, string calldata description, uint256 price) external {
        // Recomendable comprobar que el vendedor no se encuentre en la blacklist
        require(offered_items + 1 > offered_items, "Overflow" ); // Evita overflow
    ...
    function disputeSale(uint256 itemId, string calldata buyerReasoning) external {    
        require(offered_items[itemId].state == State.Sold, "The item has to be in Sold state previously");
        require(offered_items[itemId].buyer == msg.sender, "The msg.sender has to be the item buyer");
        /*
    ...
    function reimburse(uint256 itemId) external onlyOwner {
        require(offered_items[itemId].seller != address(0), "itemId does not exist");
        require(offered_items[itemId].state == State.Disputed, "The item has to be in Disputed state previously");
        require(offered_items[itemId].buyer == msg.sender, "The msg.sender has to be the item buyer");
    ...
    function itemReceived(uint256 itemId) external { 
        require(offered_items[itemId].state == State.Pending, "The item has to be in Pending state previously");
        require(offered_items[itemId].buyer == msg.sender, "The msg.sender has to be the item buyer");
        /*
    ...
    function deleteSale(uint256 itemId, bool toBePaid) public {
        require(offered_items[itemId].seller = msg.sender, "Only seller can delete him offered item");  //check
        require(offered_items[itemId].state == State.Sold, "The item has to be in Sold state previously");
